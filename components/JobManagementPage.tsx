...
// Build and fetch jobs from searchJobs with filters
const fetchJobsWithFilters = async () => {
  try {
    ...
    const token = typeof window !== 'undefined' && localStorage.getItem('jdp_auth')
      ? JSON.parse(localStorage.getItem('jdp_auth') as string).token
      : null;
    ...
    // Map UI values to API expected ones
    const typeParam = filterType !== 'all' ? (
      filterType === 'service-based' ? 'service_based' : filterType === 'contract-based' ? 'contract_based' : filterType
    ) : '';
    if (typeParam) params.set('job_type', typeParam);
    ...
    const statusParam = filterStatus !== 'all' ? (statusMap[filterStatus] || filterStatus) : '';
    ...
    const url = `${apiBaseUrl}/job/searchJobs?${params.toString()}`;
    const resp = await fetch(url, { method: 'GET', headers });
    if (!resp.ok) throw new Error('Failed to fetch filtered jobs');
...
// Consolidated useEffect for all job fetching logic
useEffect(() => {
  const debounceTimeout = setTimeout(() => {
    const hasAnyFilter = filterType !== 'all' || filterStatus !== 'all' || filterPriority !== 'all' || !!searchTerm.trim();
    if (hasAnyFilter) {
...
}, [searchTerm, currentPage, filterPriority, filterStatus, filterType]);
  ...
  const handleCompleteJob = async (jobId: string) => {
    try {
      await apiClient.completeJob(jobId, 'completed')
  ...
  const confirmCompleteJob = async () => {
    ...
    try {
      ...
      await handleCompleteJob(jobToComplete)
    } finally {
      ...
      <Dialog open={showCompleteDialog} onOpenChange={setShowCompleteDialog}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmCompleteJob} disabled={isCompleting}>
